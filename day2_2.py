inStr = """Game 1: 1 red, 10 blue, 5 green; 11 blue, 6 green; 6 green; 1 green, 1 red, 12 blue; 3 blue; 3 blue, 4 green, 1 red
...
inTest = '''Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green'''

def roundReq(lineRounds):
    result = [0,0,0]
    r = 0
    g = 0
    b = 0
    colours = ['red', 'green','blue']
    for i,f in enumerate(lineRounds):
        balls = lineRounds[i].split(", ")
        for d,f in enumerate(balls):
            for idx, colour in enumerate(colours):
                
                if balls[d].find(colour) != -1:
                    if result[idx]<int(balls[d][0:2]):
                        result[idx] = int(balls[d][0:2])
    return result
...
for i, o in enumerate(lines):
    ...
    lines[i] = [lines[i][0][5:], lines[i][1][1:]]
    for a in lines[i][1:]:    
        lineRounds = a.split('; ')
        lines[i][1] = roundReq(lineRounds)
    
constraint = [12, 13, 14]

result = 0
for line in lines:
    result += line[1][0]*line[1][1]*line[1][2]
