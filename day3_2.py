...
strIn = '''.......358..........31.....339.....669.............598......328.....575......................447..650..............964...........692........
...
def getGearRatio(y,x):
    surroundings = [[y+1, x],[y, x+1],[y-1, x],[y, x-1],[y+1, x+1],[y-1, x-1],[y+1, x-1],[y-1, x+1]]
    #surroundings = [o for o in surroundings if o[0] <= shape[0] and o[1] <= shape[1] and o[0] >= 0 and o[1] >= 0]
    for i, coord in enumerate(surroundings):
        if coord[0] <= shape[0] and coord[1] <= shape[1] and coord[0] >= 0 and coord[1] >= 0:
            surroundings[i] = islands[coord[0]][coord[1]]
        else:
            surroundings[i] = islands[y][x]

    found = set()
    for i in surroundings:
        if str(i).isdigit() and i != 0:
            found.add(i)
    if len(found) == 2: ### A WAY TO FIND THE ACTUAL DATA AND RETURN IT INSTEAD OF ISLAND NUMBERS
        # found 2 islands, track the ids
        found = list(found)
        a = returnIsland(found[0])
        b = returnIsland(found[1])
        ares = 0
        bres = 0
...
def getNum(y,x):
    result = []
    if x != 0:
        while str(data[y][x]).isdigit() and x >= 0:
            x-=1
        x+=1
...
for i, o in enumerate(data):
    for j, o2 in enumerate(o):
        if data[i][j] != '.' and data[i][j] != '*' and not str(data[i][j]).isdigit():        
            data[i][j] = '.'


def markIsland(y,x, count):
    result = []
    if islands[y][x] != 0:
        return 0
    if x != 0:
...
result = 0
for y, o in enumerate(data):
    for x, o2 in enumerate(o):
        if data[y][x] == '*':        
            result += getGearRatio(y,x)
